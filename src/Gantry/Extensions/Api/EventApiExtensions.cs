...
public static class EventApiExtensions
{
    ...
    /// </summary>
    /// <param name="core">Provides access to core gantry services.</param>
    /// <param name="eventApi">The event API instance used to enqueue the action on the main thread.</param>
    ...
    /// </remarks>
    public static void AwaitMainThreadTask(this IEventAPI eventApi, ICoreGantryAPI core, Action action)
    {
        // Server is shutting down.
        if (core.ApiEx.MainThread is null) return;

        // Check if already on the main thread, and execute inline if so.
        if (Environment.CurrentManagedThreadId == core.ApiEx.MainThread.ManagedThreadId)
        {
            action();
            return;
        }
        ...
        eventApi.EnqueueMainThreadTask(() =>
        {
            ...
            {
                core.Logger.Error("Error in main thread task: " + ex);
                throw;
            }
            finally
            {
        ...
        {
            waitHandle.Set();
            core.Logger.Error("Main thread task timed out.");
