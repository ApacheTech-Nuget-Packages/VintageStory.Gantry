...
namespace Gantry.GameContent.GUI;
...
public sealed class MessageBox : GenericDialogue
{
    ...
    /// </summary>
    /// <param name="api">The <see cref="ICoreGantryAPI"/> to use for the message box.</param>
    ...
    private MessageBox(ICoreGantryAPI api, string title, string message, ButtonLayout buttons) : base(api)
    {
        var defaultTitle = (_buttons = buttons) switch
        {
            ButtonLayout.Ok => Gantry.Lang.Confirmation("information"),
            ButtonLayout.OkCancel => Gantry.Lang.Confirmation("confirmation"),
